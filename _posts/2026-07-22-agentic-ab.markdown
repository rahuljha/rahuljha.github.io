...
Agentic A/B testing doesn't need to completely replace real A/B tests in order to be useful. The more immediate use is as a gate: run a proposed change past a population of agents first, and use the result to prune which ideas are actually worth building and testing on real users, instead of putting every candidate through a multi-week rollout. It can also run alongside a live test rather than instead of it, adding a layer of insight standard A/B testing doesn't give you on its own: a reason for the effect, not just the size of it. Used this way, agentic A/B testing doesn't have to win the argument about whether it can fully replace a real experiment. It just has to be useful enough to run before, or next to, one.
